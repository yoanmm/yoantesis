<template>
  <div ref="form_container" class="card position-relative mb-0" v-on:keyup.enter="save_model()">
    ...
    <div class="card-body p-0">
      ...
      <tc-form :feedbacks="mb.statics('Compromiso_participacion').feedbacks" :vobject="$v"
        nested="compromiso_participacion" ref="form" class="form-row">
        <!-- Campo: Delegacion Evento -->
        <tc-form-item class="form-group mb-0 col-md-6 px-3">
          <label>Delegación Evento</label>
          <div class="d-flex flex-row">
            <tc-autocomplete placeholder="Seleccione la Delegación Evento" name="id_delegacion_evento"
              ref="select_delegacion_evento" :successFeed="false" idKey="id_delegacion_evento" textKey="nombre"
        ...
        <a-modal @cancel="showModalCreatedelegacion_evento = false" v-if="showModalCreatedelegacion_evento"
          class="modal-form" :title="'Añadir Delegación Evento'" :visible="true" :header="null" :footer="null"
          :maskClosable="false">
        ...
        <tc-form-item class="form-group mb-0 col-md-6 px-3">
          <label>Deporte</label>
          <div class="d-flex flex-row">
            <tc-autocomplete placeholder="Seleccione los Deportes" name="id_deporte" ref="select_deporte"
              :successFeed="false" idKey="id_deporte" textKey="nombre_deporte"
              :defaultValue="compromiso_participacion.id_deporte" v-model="compromiso_participacion.id_deporte"
              :url="mb.statics('Deporte').select_2_url" multiple />
            <!--<a-button type="dashed"
                      icon="plus"
                      class="dashed-primary rounded mt-1 ml-2"
        ...
        <a-modal @cancel="showModalCreatedeporte = false" v-if="showModalCreatedeporte" class="modal-form"
          :title="'Añadir Deporte'" :visible="true" :header="null" :footer="null" :maskClosable="false">
    ...
    <div class="card-footer p-0">
      <a-button-group style="margin-bottom: 10px;margin-top: 10px">
        <a-tooltip placement="topLeft"
          :title="!compromiso_participacion.get_id() ? 'Añadir nuevo elemento' : 'Actualizar elemento'">
        ...
        <a-tooltip placement="topLeft" title="Añadir y resetear para insertar nuevo elemento">
          <a-button v-if="!compromiso_participacion.get_id()" :disabled="loading" type="primary"
            @click="save_model(true)">Añadir y nuevo</a-button>
        </a-tooltip>
...
export default {
  ...
  methods: {
    ...
    save_model(and_new = false) {
      ...
      // Normaliza respuestas: a veces viene en response.data (axios), otras viene plano
        ...
        Promise.all(promises)
          .then(responses => {
            responses.forEach(resp => {
              if (isDuplicate(resp)) {
                utils.openNotificationWithIcon('warning', 'Duplicado detectado', getMessage(resp));
              } else {
                ...
                if (data && data.model) {
                  utils.openNotificationWithIcon(
                    'success',
                    'Compromiso añadido',
                    `Se agregó correctamente el deporte ID ${data.model.id_deporte}.`
                  );
                } else {
                  utils.openNotificationWithIcon(
                    'success',
          ...
          .then((resp) => {
            ...
            if (data && data.success === false) {
              ...
              utils.openNotificationWithIcon(
                'success',
                'Compromiso añadido',
                `Se agregó '${data.deporte}' para '${data.delegacion_evento}'.`
              );
            }
