<template>
  ...
  >
   ...
   <a-menu-item key="2">
    ...
    <slot :slot="'nombre'" slot-scope='record' name='nombre' :record='record'/>
    <slot :slot="'apellido'" slot-scope='record' name='apellido' :record='record'/>
    <slot :slot="'carnet'" slot-scope='record' name='carnet' :record='record'/>
...
export default {
  ...
  methods: {
    ...
    onEditing(model) {
      ...
      this.setSelectedPersona(this.selected_model)
      this.show_form();
    }
  },
  mounted() {
