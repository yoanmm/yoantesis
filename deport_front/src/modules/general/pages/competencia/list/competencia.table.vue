<template>
  ...
  >
    ...
    <slot :slot="'resultado_competencia'" slot-scope='record' name='resultado_competencia' :record='record'/>
    <slot :slot="'terminado'" slot-scope='record' name='terminado' :record='record'/>
    <slot :slot="'fin_inscripcion'" slot-scope='record' name='fin_inscripcion' :record='record'/>
  </a-table>
</template>
...
export default {
  ...
  methods: {
    ...
    onEditing(model) {
      ...
      this.setSelectedCompetencia(this.selected_model)
      this.show_form();
    }
  },
  mounted() {
