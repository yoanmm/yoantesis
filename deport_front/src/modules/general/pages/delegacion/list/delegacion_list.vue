<template>
  <div>
   ...
   <div>
      <a-modal
        @cancel="onCloseModal"
        :title="selected_delegacion.get_id()?'Actualizar delegacion':'Añadir delegacion'"
        class="modal-form"
        width="55rem"
  ...
  methods: {
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Delegacion').delete_by_ids(
