<template>
  ...
  >
   ...
   <a-menu-item key="2">
    ...
    <slot :slot="'nombre_delegacion'" slot-scope='record' name='nombre_delegacion' :record='record'/>
    <slot :slot="'abreviatura'" slot-scope='record' name='abreviatura' :record='record'/>
    <slot :slot="'mascota'" slot-scope='record' name='mascota' :record='record'/>
...
export default {
  ...
  methods: {
    ...
    onEditing(model) {
      ...
      this.setSelectedDelegacion(this.selected_model)
      this.show_form();
    }
  },
  mounted() {
