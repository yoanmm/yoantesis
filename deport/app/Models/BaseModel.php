...
use Modules\security\Services\LogService;
...
class BaseModel extends Model
{
    ...
    protected static function boot()
    {
        ...
        static::updated(function (Model $model) {
            if ($model->table != 'log') {
                LogService::createAny('Actualizado', $model->table, $model->getKey());
            }
        });
        static::created(function (Model $model) {
            if ($model->table != 'log') {
                LogService::createAny('Creado', $model->table, $model->getKey());
            }
        });
        static::deleted(function (Model $model) {
            if ($model->table != 'log') {
                LogService::createAny('Eliminado', $model->table, $model->getKey());
            }
        });
    }
}
